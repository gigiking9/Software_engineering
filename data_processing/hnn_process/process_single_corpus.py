...
def load_pickle(filename):
    file_data=pickle.load(open(filename, 'rb'), encoding='iso-8859-1')
    return file_data
...
def staqc_single_multiples(filepath,save_single_path,save_mutiple_path):
...
def large_single_multiples(filepath,save_single_path,save_mutiple_path):
...
#把单候选只保留其qid
def single_unlable_lable(path1,path2):
...
if __name__ == "__main__":
    ...
    staqc_single_multiples(staqc_python_path,staqc_python_sigle_save,staqc_python_multiple_save)
    ...
    staqc_single_multiples(staqc_sql_path, staqc_sql_sigle_save, staqc_sql_multiple_save)
    ...
    large_single_multiples(large_python_path, large_python_single_save, large_python_multiple_save)
    ...
    single_unlable_lable(large_sql_single_save,large_sql_single_label_save)
    single_unlable_lable(large_python_single_save, large_python_single_label_save)
