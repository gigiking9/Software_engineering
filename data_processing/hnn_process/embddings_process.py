...
def trans_bin(path1,path2):
    word_from_text = KeyedVectors.load_word2vec_format(path1, binary=False)
    # 如果每次都用上面的方法加载，速度非常慢，可以将词向量文件保存成bin文件，以后就加载bin文件，速度会变快
    word_from_text.init_sims(replace=True)
    word_from_text.save(path2)
    '''n
    读取用一下代码
...
def Serialization(word_dict_path, type_path, final_type_path):
    ...
    for i in range(len(corpus)):
        ...
        tokenized_code = tokenized_code[:350] if len(tokenized_code) >= 350 else tokenized_code + [0] * (350 - len(tokenized_code))
